...
func init() {
	// Set lipgloss color profile based on terminal capabilities
	if noColor {
...
// getSymbol returns the appropriate symbol based on TTY capability.
func getSymbol(unicode, ascii string) string {
	if isTTYStdout {
		return unicode
	}
	return ascii
