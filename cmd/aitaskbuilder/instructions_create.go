...
// InstructionsCreateOptions are the options for creating AI Task Builder instructions.
type InstructionsCreateOptions struct {
	Args             []string
	BatchID          string
...
// NewInstructionsCreateCommand creates a new command for creating AI Task Builder instructions.
func NewInstructionsCreateCommand(client client.API, w io.Writer) *cobra.Command {
	var opts InstructionsCreateOptions

	cmd := &cobra.Command{
		Use:   "create",
		Short: "Create instructions for an AI Task Builder batch",
		Long: `Create instructions for an AI Task Builder batch

This command allows you to create instructions for a specific AI Task Builder batch.
Instructions define the tasks that participants will complete. You can provide instructions
either as a JSON file or as a JSON string directly.
...
- multiple_choice_with_free_text: Instructions with options and text input`,
		Example: `
Create instructions from a file:
$ prolific aitaskbuilder instructions create -b <batch_id> -f instructions.json

Create instructions with JSON string:
$ prolific aitaskbuilder instructions create -b <batch_id> -j '[{"type":"free_text","created_by":"Sean","description":"Please explain your choice."}]'
  ...
  },
  {
    ...
    "description": "Please share the reasons for your choice."
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			err := createInstructions(client, opts, w)
			if err != nil {
	...
	flags := cmd.Flags()
	flags.StringVarP(&opts.BatchID, "batch-id", "b", "", "Batch ID (required) - The ID of the batch to create instructions for.")
...
// createInstructions will create instructions for an AI Task Builder batch
func createInstructions(c client.API, opts InstructionsCreateOptions, w io.Writer) error {
	if opts.BatchID == "" {
	...
	fmt.Fprintf(w, "Successfully created instructions for batch %s\n", opts.BatchID)
	if response.Message != "" {
