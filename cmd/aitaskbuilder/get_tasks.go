...
type BatchGetTasksOptions struct {
	Args    []string
	BatchID string
}

func NewGetTasksCommand(client client.API, w io.Writer) *cobra.Command {
	var opts BatchGetTasksOptions
...
$ prolific aitaskbuilder task gettasks -b <batch_id>
		`,
...
func renderAITaskBuilderTasks(c client.API, opts BatchGetTasksOptions, w io.Writer) error {
	if opts.BatchID == "" {
